...
$app->post('/bot', function() use($app) {
	...
	switch($data->type)
	{
		...
		case 'message_new':
			...
			$message = preg_replace("|[^\d\w ]", "", $message);
			$date = date("d.m.Y  H:i");
			$messages_array = [
				...
				'дуц, дата?' => "[id{$user_id}|{$user_name}], {$date} +3 часа сверху.=)",
				'дуц, кто такой Фил?' => "[id{$user_id}|{$user_name}], челик, который проебался с арендой тачки.=)",
				'дуц, кто такой Макс?' => "[id{$user_id}|{$user_name}], заебис чел, битки там, хуё-моё, [id34317520|Krzhprd.] в общем.=)",
				'дуц, кто такой Даня?' => "[id{$user_id}|{$user_name}], тупо [id20017026|создатель].",
				'дуц, споки ноки' => "[id{$user_id}|{$user_name}], бархатной ночи, вы закроете глаза, а моя душа будет сидеть у ваших ног и охранять сны до рассвета, нежно улыбаясь вам. ^_^",
				'дуц, когда лабы?' => "[id{$user_id}|{$user_name}],\nКванты:\n9 апреля\n13-15: Мураш, Фил, Кузьмич, Комлева, Кузин\n15:40 - 19: все остальные\nУГИФС:\n15 апреля(первая подгруппа)\n23 апреля(вторая подгруппа)\n21 мая - защита(вторая подгруппа)\n27 мая - защита(первая подгруппа)",
				'дей дуц' =>  "Чё кого, [id{$user_id}|{$user_name}]?",
				'дуц, заебис. =)' =>  "Это по-кайфу, [id{$user_id}|{$user_name}].=)"
