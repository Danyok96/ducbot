	...
	switch($data->type)
	{
		...
		case 'message_new':
			...
			$messages_array = [
				...
				'дуц, кто такая ирина?' =>"[id{$user_id}|{$user_name}-дуц], 'чудесная и хорошая'(с) [id201182825|Иринус]. =)",
				'дуц, кто такой петёёёк?' => "[id{$user_id}|{$user_name}-дуц], ну просто максимальный [id14806435|Дуц-танкист], который ещё и братков из группы кинул и перешёл в другую группу...",
				'дуц, кто такой фирст?' =>"[id{$user_id}|{$user_name}-дуц], чики-бирики и в [id117803113|чела на BMW]. =)",
				'дуц, кто такой гвоздь?' => "[id{$user_id}|{$user_name}-дуц], просто [id174929520|Doge with the gun]. =)",
				...
				'бунд' => "[id{$user_id}|{$user_name}-дуц], так, блэт. Успокаеваемся!=)"
