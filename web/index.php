...
$app->post('/bot', function() use($app) {
	...
	switch($data->type)
	{
		...
		case 'message_new':
			...
			switch ($message) {
				case 'приветдуц':
					$otvet = "Привет, [id{$user_id}|{$user_name}] !";
					break;
				case 'дуцкакдела':
					$otvet = "[id{$user_id}|{$user_name}], збс, ведь я не учусь.=)";
					break;
				case 'дуцчтоумеешь':
					$otvet = "[id{$user_id}|{$user_name}], кидать подгоны, чтобы Даня не агрился.=)";
					break;
				case 'дуцктотакойходж':
					$otvet = "[id{$user_id}|{$user_name}], нормас чел, иногда сутулый, конечно, но ладно.=)";
					break;
				case 'дуцдата':
					$otvet = "[id{$user_id}|{$user_name}], {$date}.\nНеделя: {$numofweek}.\nУчебная неделя: {$numofstudweek}.\nЗавтра: {$nextdate}.";
					break;
				case 'дуцвремя':
					$otvet = "[id{$user_id}|{$user_name}], {$time}.";
					break;
				case 'дуцктотакойфил':
					$otvet = "[id{$user_id}|{$user_name}], челик, который проебался с арендой тачки.=)";
					break;
				case 'дуцктотакоймакс':
					$otvet = "[id{$user_id}|{$user_name}], заебис чел, битки там, хуё-моё, [id34317520|Krzhprd.] в общем.=)";
					break;
				case 'дуцктотакойданя':
					$otvet = "[id{$user_id}|{$user_name}], тупо [id20017026|создатель].";
					break;
				case 'дуцктотакаяирина':
					$otvet = "[id{$user_id}|{$user_name}], 'чудесная и хорошая'(с) [id201182825|Иринус]. =)";
					break;
				case 'дуцктотакойпетёёёк':
					$otvet = "[id{$user_id}|{$user_name}], ну просто максимальный [id14806435|Дуц-танкист], который ещё и братков из группы кинул и перешёл в другую группу...";
					break;
				case 'дуцктотакойфирст':
					$otvet = "[id{$user_id}|{$user_name}], чики-бирики и в [id117803113|чела на BMW]. =)";
					break;
				case 'дуцктотакойгвоздь':
					$otvet = "[id{$user_id}|{$user_name}], просто [id174929520|Doge with the gun]. =)";
					break;
				case 'дуцспокиноки':
					$otvet = "[id{$user_id}|{$user_name}], бархатной ночи, вы закроете глаза, а моя душа будет сидеть у ваших ног и охранять сны до рассвета, нежно улыбаясь вам. ^_^";
					break;
				case 'дуцкогдалабы':
					$otvet = "[id{$user_id}|{$user_name}],\nУГИФС:\n15 апреля(первая подгруппа)\n23 апреля(вторая подгруппа)\n21 мая - защита(вторая подгруппа)\n27 мая - защита(первая подгруппа)";
					break;
				case 'хейдуц':
					$otvet = "Чё кого, [id{$user_id}|{$user_name}]?";
					break;
				case 'дуцзаебис':
					$otvet = "Это по-кайфу, [id{$user_id}|{$user_name}].=)";
					break;	
				case 'бунд':
					$otvet = "[id{$user_id}|{$user_name}], так, блэт. Успокаеваемся!=)";
					break;
				case 'дуцрасписаниенасегодня':
						if(($numofstudweek % 2) == 0)
						{
							...
							switch ($day) {
								case 'Sun':
									$otvet = "[id{$user_id}|{$user_name}], чиль, сегодня выходной. =)";
									break;
								case 'Mon':
									$otvet = "[id{$user_id}|{$user_name}], сегодня только военка первой парой.";
									break;
								case 'Tue':
									if($date == '09.04.2019')
									{
										$otvet = "[id{$user_id}|{$user_name}], сегодня лабы по квантам.";
									}
									else
									{
										$otvet = "[id{$user_id}|{$user_name}], чиль, сегодня выходной. =)";
						...
						else
						{
							...
							switch ($day) {
								...
								case 'Mon':
								if ($date == '15.04.2019') {
									$otvet = "[id{$user_id}|{$user_name}], сегодня лабы в п.Орево (первая подгруппа) и военка.=)";
								} else {
									$otvet = "[id{$user_id}|{$user_name}], сегодня только военка первой парой.";
								}
									break;
								case 'Tue':
								if ($date == '23.04.2019') {
									$otvet = "[id{$user_id}|{$user_name}], сегодня лабы в п.Орево (вторая подгруппа) и военка.=)";
								} else {
									$otvet = "[id{$user_id}|{$user_name}], чиль, сегодня выходной. =)";								
								}
									break;
				...
				case 'дуцрасписаниеназавтра':
						if(($numofnextstudweek % 2) == 0)
						{
							...
							switch ($nextday){
								case 'Sun':
									$otvet = "[id{$user_id}|{$user_name}], чиль, выходной. =)";
									break;
								case 'Mon':
									$otvet = "[id{$user_id}|{$user_name}], сегодня только военка первой парой.";
									break;
